...
//            $message = $this->coreParametersHelper->getParameter('resubscribe_message');
    ...
    public function previewAction($objectId)
    {
        ...
        if($content != null) {
            ...
            if (($body and $body->length > 0) && ((strpos($content, '{{global_unsubscribe_link}}') == 0) && (strpos($content, '{unsubscribe_link}') == 0) && (strpos($content, '{update_your_profile_link}') == 0))&& $emailEntity->getTemplate() !='template') {
                $body = $body->item(0);
