...
Le.emailOnLoad = function (container, response) {
    ...
    mQuery('.next-tab, .prev-tab, .ui-state-default').click(function() {
        ...
        if(!mQuery('#email-advance-container').hasClass('hide')) {
            ...
            if (url.indexOf('oneoff/new') == -1 && mQuery('textarea.bee-editor-json').val() != "RichTextEditor") {
...
Le.selectEmailEditor = function(editorType) {
    ...
    if (editorType == 'basic' || editorType == 'code') {
        ...
        mQuery('#next-tab-3').attr('style','margin-left:78%;');
        mQuery('#previous-button-3').attr('style','margin-left:-18%;');
        mQuery('#email-2-button-div').attr('style','margin-right:120px;');
