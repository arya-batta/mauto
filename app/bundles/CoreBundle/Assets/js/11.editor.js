Le.builderTokens = {};
Le.dynamicContentTokens = {};
Le.builderTokensRequestInProgress = false;
...
Le.activateGlobalFroalaOptions = function() {
    Le.basicFroalaOptions = {
        ...
        imageUploadURL: leBaseUrl + 'a/file/upload',
        imageManagerLoadURL: leBaseUrl + 'a/file/list',
        imageManagerDeleteURL: leBaseUrl + 'a/file/delete',
        imageDefaultWidth: 0,
        pastePlain: true,
 ...
 */
Le.initAtWho = function(element, method, froala) {
    ...
    if (Le.builderTokensRequestInProgress) {
        ...
        var intervalID = setInterval(function(){
            if (!Le.builderTokensRequestInProgress) {
                ...
                Le.configureAtWho(element, method, froala);
            }
        }, 500);
    } else {
        Le.configureAtWho(element, method, froala);
...
Le.configureAtWho = function(element, method, froala) {
    Le.getTokens(method, function(tokens) {
        ...
        mQuery.extend(tokens, Le.dynamicContentTokens);
 ...
 */
Le.getTokens = function(method, callback) {
    // Check if the builderTokens var holding the tokens was already loaded
    if (!mQuery.isEmptyObject(Le.builderTokens)) {
        return callback(Le.builderTokens);
    }

    Le.builderTokensRequestInProgress = true;
    ...
    mQuery.ajax({
        ...
        success: function (response) {
            if (typeof response.tokens === 'object') {
                ...
                Le.builderTokens = response.tokens;
 ...
 */
Le.configureDynamicContentAtWhoTokens = function() {
    Le.dynamicContentTokens = {};
    ...
    if (dynamicContentTabs.length) {
        dynamicContentTabs.find('a[data-toggle="tab"]').each(function () {
            ...
            Le.dynamicContentTokens[newOption] = tokenText;
        });
    }
};
