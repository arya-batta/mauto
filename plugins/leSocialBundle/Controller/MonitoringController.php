...
namespace MauticPlugin\leSocialBundle\Controller;

use Mautic\CoreBundle\Controller\FormController;
use Mautic\CoreBundle\Helper\Chart\LineChart;
use Mautic\LeadBundle\Controller\EntityContactsTrait;
use MauticPlugin\leSocialBundle\Entity\Monitoring;
...
class MonitoringController extends FormController
{
    ...
    {
        if (!$this->container->get('mautic.security')->isGranted('plugin:leSocial:monitoring:create')) {
        ...
        if ($method == 'POST') {
            ...
            $template       = 'leSocialBundle:Monitoring:index';
            ...
            if (!$cancelled = $this->isFormCancelled($form)) {
                if ($valid = $this->isFormValid($form)) {
                    ...
                    $template = 'leSocialBundle:Monitoring:view';
                }
            }
    ...
    {
        if (!$this->container->get('mautic.security')->isGranted('plugin:leSocial:monitoring:edit')) {
            return $this->accessDenied();
        }
                ...
                return $this->postActionRedirect(
                    array_merge(
                        ...
                        [
                            ...
                            'viewParameters'  => $viewParameters,
                            'contentTemplate' => 'leSocialBundle:Monitoring:view',
                        ]
                    )
                );
            }
    ...
    {
        if (!$this->get('mautic.security')->isGranted('plugin:leSocial:monitoring:view')) {
            return $this->accessDenied();
        }
        ...
        /** @var \MauticPlugin\leSocialBundle\Entity\PostCountRepository $postCountRepo */
            ...
            [
                ...
                'viewParameters' => [
                    ...
                    'monitorLeads'     => $this->forward(
                        'leSocialBundle:Monitoring:contacts',
                        [
                            'objectId'   => $monitoringEntity->getId(),
                            'page'       => $page,
                            'ignoreAjax' => true,
    ...
    {
        if (!$this->get('mautic.security')->isGranted('plugin:leSocial:monitoring:delete')) {
            return $this->accessDenied();
        }
        ...
        if ($this->request->getMethod() == 'POST') {
            ...
            if ($entity === null) {
                ...
                return $this->isLocked($postActionVars, $entity, 'plugin.leSocial.monitoring');
            }
    ...
    {
        if (!$this->container->get('mautic.security')->isGranted('plugin:leSocial:monitoring:delete')) {
            return $this->accessDenied();
        }
    ...
    public function contactsAction($objectId, $page = 1)
    {
        return $this->generateContactsGrid(
            $objectId,
            $page,
            'plugin:leSocial:monitoring:view',
            'social',
            'monitoring_leads',
