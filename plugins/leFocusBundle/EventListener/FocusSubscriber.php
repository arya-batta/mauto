...
namespace MauticPlugin\leFocusBundle\EventListener;
...
use MauticPlugin\leFocusBundle\Model\FocusModel;
...
class FocusSubscriber extends CommonSubscriber
{
    ...
    public function onKernelRequest(GetResponseEvent $event)
    {
        if ($event->isMasterRequest()) {
            ...
            $formGenerateUrl = $this->router->generate('le_form_generateform');

            if (strpos($requestUri, $formGenerateUrl) !== false) {
                $id = InputHelper::_($this->request->get('id'));
                if (strpos($id, 'mf-') === 0) {
                    $mfId             = str_replace('mf-', '', $id);
                    $focusGenerateUrl = $this->router->generate('le_focus_generate', ['id' => $mfId]);
